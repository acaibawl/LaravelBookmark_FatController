...
class ShowBookmarkListPageUseCaseTest extends TestCase
{
  ...
  public function testResponseIsCorrect()
  {
     ...
     * 
     * 対象ファサードの対象メソッドを、どんな引数で何回呼ばれるかをテストできる
     */
    SEOTools::shouldReceive('setTitle')->withAnyArgs(['ブックマーク一覧'])->once();
    SEOTools::shouldReceive('setDescription')->withAnyArgs(['技術分野に特化したブックマーク一覧です。みんなが投稿した技術分野のブックマークが投稿順に並んでいます。HTML、CSS、JavaScript、Rust、Goなど、気になる分野のブックマークに絞って調べることもできます'])->once();
